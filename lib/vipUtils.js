...
export async function triggerContractAndPaymentLink(matchId) {
    ...
    // NOTE: Tutor commission is deferred — will be handled via subscription credits
    // when the tutor payment model is built. For now we just create the contract
    // and notify both parties. The paymentLinkUrl stays null.
    const paymentLinkUrl = null;

    // Email tutor with student contact details (commission handled separately via credits)
    await sendEmail(tutor.email, `New VIP Student Confirmed — ${student.name}`,
...
function buildTutorCommissionEmailHtml({ tutorName, studentName, subjects, studentPhone, studentEmail }) {
    return emailWrapper(`
        ...
        <p style="color: #6b7280;">
            ...
            Here are their contact details — reach out to schedule your first session.
        </p>
        <div style="background: #f0f7ff; border-radius: 8px; padding: 20px; margin: 16px 0;">
        ...
        <div style="text-align: center; margin-top: 24px;">
            <a href="${BASE_URL}/dashboard/tutor?tab=vip"
               style="background: #2563eb; color: white; padding: 14px 32px; border-radius: 8px; text-decoration: none; font-weight: bold; font-size: 15px;">
                View in Dashboard
            </a>
        </div>
        <p style="color: #9ca3af; font-size: 12px; margin-top: 16px;">
            If you have any issues, contact support@tuitionsinindia.com
        </p>
    `);
}

...
const _secret = process.env.SESSION_SECRET || process.env.AUDIT_SEED_KEY;
if (!_secret) {
    if (process.env.NODE_ENV === "production") {
        throw new Error("SESSION_SECRET env var is required in production. Set it in .env.production.");
    }
    console.warn("[session] SESSION_SECRET not set — using insecure development fallback.");
}
const SESSION_KEY = _secret || "tuitions-dev-only-secret-do-not-use-in-prod";
...
function signPayload(payload) {
    ...
    const sig = crypto.createHmac("sha256", SESSION_KEY).update(data).digest("hex");
    return `${data}.${sig}`;
}
...
export function verifyToken(token) {
    ...
    const expectedSig = crypto.createHmac("sha256", SESSION_KEY).update(data).digest("hex");

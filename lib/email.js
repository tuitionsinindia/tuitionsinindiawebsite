...
export const sendTrialNotificationEmail = async (toEmail, toName, { status, subject, otherPartyName, dashboardLink }) => {
    try {
        const subjectLines = {
            REQUESTED: `New demo class request for ${subject}`,
        ...
        const bodyLines = {
            REQUESTED: `Hi ${toName}, a student (<strong>${otherPartyName}</strong>) has requested a demo class for <strong>${subject}</strong>. Log in to your dashboard to accept or decline the request.`,
            ...
            DECLINED: `Hi ${toName}, your demo class request for <strong>${subject}</strong> was not accepted by <strong>${otherPartyName}</strong>. You can try booking a demo class with another tutor.`,
...
export const sendVerificationResultEmail = async (toEmail, toName, { approved, rejectionReason }) => {
    try {
        ...
        const emailBody = approved
            ? `Hi ${toName}, congratulations! Your profile has been reviewed and you now have a verified badge on TuitionsInIndia. Your listing will appear higher in search results and students are more likely to contact verified tutors.`
            : `Hi ${toName}, your verification request was reviewed but could not be approved at this time. Reason: <strong>${rejectionReason || "Please contact support for details."}</strong><br><br>You are welcome to reapply after addressing the issue. If you have questions, reply to this email.`;

        const ctaText = approved ? "View Your Profile" : "Update and Reapply";
        const ctaLink = "https://tuitionsinindia.com/dashboard/tutor";
...
<tr><td style="padding:32px">
<h2 style="margin:0 0 12px;color:#111827;font-size:20px">${approved ? "Profile Verified ✓" : "Verification Update"}</h2>
<p style="margin:0 0 20px;color:#6b7280;font-size:14px;line-height:1.6">${emailBody}</p>
<a href="${ctaLink}" style="display:inline-block;padding:12px 28px;background:#2563EB;color:#ffffff;text-decoration:none;border-radius:10px;font-weight:600;font-size:14px">${ctaText}</a>
</td></tr>
<tr><td style="padding:16px 32px;border-top:1px solid #f1f5f9;text-align:center">
<p style="margin:0;color:#9ca3af;font-size:12px">TuitionsInIndia — India's Trusted Tutor Discovery Platform</p>
</td></tr>

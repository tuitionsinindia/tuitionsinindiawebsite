...
export const sendTrialNotificationEmail = async (toEmail, toName, { status, subject, otherPartyName, dashboardLink }) => {
    try {
        const subjectLines = {
            REQUESTED: `New trial class request for ${subject}`,
        ...
        const bodyLines = {
            REQUESTED: `Hi ${toName}, a student (<strong>${otherPartyName}</strong>) has requested a free trial class for <strong>${subject}</strong>. Log in to your dashboard to accept or decline the request.`,
